...
/*
 * BORE derives sysctl_sched_base_slice from HZ and sysctl_sched_min_base_slice,
 * so tunable_scaling (and sched_update_scaling()) no longer apply. Expose the
 * writable knob as min_base_slice_ns instead; base_slice_ns becomes read-only.
 * The same values are also reachable via /proc/sys/kernel/ -- see
 * kernel/sched/bore.c.
 */
static ssize_t sched_min_base_slice_write(struct file *filp,
