 ...
 * (default min_base_slice = CONFIG_MIN_BASE_SLICE_NS, units: nanoseconds)
...
#ifdef CONFIG_SCHED_BORE
	    && !(likely(sched_bore) && likely(sched_burst_parity_threshold) &&
...
place_entity(struct cfs_rq *cfs_rq, struct sched_entity *se, int flags)
{
	...
	if (likely(sched_bore)) {
		/*
		 * BORE: shorten the initial deadline for the enqueue reasons
		 * listed in sched_deadline_boost_mask, so a task that just
		 * woke up or was just forked gets to run sooner.
		 */
		vslice >>= !!(flags & sched_deadline_boost_mask);
	} else if (sched_feat(PLACE_DEADLINE_INITIAL) &&
...
#ifdef CONFIG_SCHED_BORE
	/*
	 * BORE: close out the burst (and rescale the deadline) before the
	 * early return, so a yielding task that is alone on the rq still
	 * gets its penalty revolved.
	 */
